...
一套**自托管**的公募基金投研工具：从公开数据源拉取基金名单、详情指标、持仓与历史净值，在此之上完成 **筛选 → 预设镜像 → 行业暴露聚类 → 簇级仓位建议 → 实盘对账/再平衡** 的完整闭环，并通过 **CLI + API** 把全部核心能力开放给本机 AI agent（如 OpenClaw），让 agent 在你不在电脑前时也能帮你管理基金。
...
- **对外集成**：通过 CLI（`backend/ifund_cli.py`）与带 PAT 认证的 HTTP API 把上述能力暴露给本机 agent。

## 🧱 技术栈

| 层 | 技术 |
| --- | --- |
| 后端 | Python 3.9 · Flask 3.1 · flask-jwt-extended · bcrypt · pydantic · akshare · waitress |
| 数据 | SQLite（多后端抽象层，预留 MySQL）|
| 前端 | React 18 · TypeScript · Ant Design 5 · Vite 5 · Tailwind |
| 集成 | CLI（ifund_cli）· httpx |
...
- 后端业务文件统一使用 `from __future__ import annotations`。
